...
import { IInputSelectBaseAttributes, InputSelectBase, InputSelectOption, NO_RESULTS_ID } from './input-select-base'
...
export class InputSelect extends InputSelectBase<ValueType> {
  ...
  protected renderReferenceElement() {
    ...
    return html`
      ...
      ${this.readonly
        ? html`<div style="display: none;"><slot @slotchange=${this.handleValueChange}></slot></div>`
        : ``}
    `
  }
  ...
  protected renderDropdownContent() {
    return html`
      ...
      ${this.options?.length ? this.renderOptionsData() : html`<slot @slotchange=${this.handleValueChange}></slot>`}
    `
  }
  ...
  private syncValueLabel(value: ValueType) {
    ...
    if (selectedItem) {
      ...
      this.selectedOptionObserver.observe(selectedItem, { characterData: false, childList: true, attributes: false })
