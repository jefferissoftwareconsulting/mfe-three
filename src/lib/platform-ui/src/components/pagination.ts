...
export class BasicPagination extends LitElement {
  ...
  static properties = {
    ...
    hasPreviousPage: { type: Boolean, attribute: 'has-previous-page', reflect: true }
  }
    ...
    return html`
      <sp-button-group role="navigation" aria-label="Pagination Navigation">
        <sp-button
          ...
          @click=${this.hasPreviousPage ? this.handlePageChange.bind(this, 'previous') : null}
        ...
        <sp-button
          ...
          @click=${this.hasNextPage ? this.handlePageChange.bind(this, 'next') : null}
          button-type="secondary"
...
class PaginationEvent extends CustomEvent<PaginationOnPageChangedEventPayload> {
  constructor(eventType: PaginationEventType, page: number) {
    super(`${pagination}-${eventType}`, { bubbles: true, composed: true, detail: { page } })
  }
}
...
export class Pagination extends LitElement {
  ...
  render() {
    ...
    this.pages = paginate(this.totalPages, this.currentPage, this.currentPageSiblings)
            ...
            ? html`<button
              class=${classMap({
                ...
                selected: this.isCurrentPage(page)
                  ...
                  role="link"
                  aria-label=${`${this.isCurrentPage(page) ? `Current Page, Page ${page}` : `Go to page ${page}`}`}
                  aria-current=${`${this.isCurrentPage(page)}`}
                  @click=${!this.isCurrentPage(page) ? this.onPageChange.bind(this, page) : null}
